...
#        major_xticks = np.arange(2.25, 7, 0.5) 
#        minor_xticks = np.arange(2.25, 7, 0.25)  
#        (xxmin,xxmax) = (3.25,5.25)
        ...
        (xxmin, xxmax) = (0, 20500)
#        major_xticks = np.arange(0, 25000, 1000) 
#        minor_xticks = np.arange(0, 21000, 500)
#        (xxmin, xxmax) = (0, 6000)
    ...
    if yvariable == 'T':
        ...
        (yymin, yymax) = (0, 20500)
#        major_yticks = np.arange(0, 25000, 1000) 
#        minor_yticks = np.arange(0, 21000, 500)
#        (yymin, yymax) = (0, 3500)
        ...
        (yymin, yymax) = (0,450)
#        major_yticks = np.arange(0, 500, 25) 
#        minor_yticks = np.arange(0, 500, 5)
#        (yymin, yymax) = (0,130)
...
def main(argv):
    ...
    try:
        ...
        print("plot_Hugoniot_all+comp.py -f <mineralname>(default = all files of every compound) -g <comparison-file> -t <type of plot: y-x variables, ex: T-rho or P-rho> -l <letter of first plot, default = ''> ")
        sys.exit()
    for opt,arg in options:
        if opt == '-h':
            print('')
            print('plot_Hugoniot_all+comp.py program to plot the Hugoniot ')
            print("plot_Hugoniot_all+comp.py -f <mineralname>(default = all files of every compound) -g <comparison-file> -t <type of plot: y-x variables, ex: T-rho or P-rho> -l <letter of first plot, default = ''> -c < =1 for comparison with thermo data points, default = 0>")
            print("plot_Hugoniot_all+comp.py requires to be lauched from the folder containing every Hugoniot .txt file created by analyze_Hugoniot.py")
            print('')
            sys.exit()
        if opt in ('-f','--fmineralname'):
            mineralname = str(arg)
        elif opt in ('-g','--gcomparisonfilename'):
            comparisonfile = str(arg)
        elif opt in ('-t','--type'):
            type_plot = str(arg)
    ...
    for file in files:
        ...
        #******* interpolation
        if xvariable == 'T' and yvariable == 'P':
            #print('plot raw data')
            ax1.plot(Hugoniot[xvariable][:],Hugoniot[yvariable][:], plot_params[mineral], 
                     marker = None, linewidth = plot_parameters["size_lines"], color =colors[gs])
        else:
            ...
            ax1.plot(xnewsmall,ynewsmall, plot_params[mineral], markersize = 0, 
                     linewidth = plot_parameters["size_lines"], color =colors[gs])
            ax1.plot(xnew,f2(xnew), plot_params[mineral], markersize = 0, 
                     linewidth = plot_parameters["size_lines"], color =colors[gs])
        if comparisonfile != '':
            pass
    ...
    try:
        ...
        for source in sources:
            for ii in range(len(compdata[source][xvariable])):
                ...
                ax1.scatter(compdata[source][xvariable][ii],compdata[source][yvariable][ii],
                            marker = markersources[source], edgecolor = colorsources[source], 
                            facecolor = fillcolor, s = plot_parameters["size_markers"]*5)
    except FileNotFoundError:
        print("Litterature file '",filename2,"' not found")
        
    #****** Create legend from custom artist/label lists  
    custom_markers = [Line2D([0], [0], marker = markersources[key], linestyle = None,
                             markeredgecolor = colorsources[key], markerfacecolor = colorsources[key], 
                             markersize = plot_parameters["size_markers"], markeredgewidth = 0.5, 
                             linewidth = 0) for key in sources]
    legend3 = ax1.legend([marker for marker in custom_markers],[label for label in sources], 
                         bbox_to_anchor=(1.05, 0.3), loc=2, fontsize = plot_parameters["size_font_ticks"],  borderaxespad=0.)
    ...
    legend2 = ax1.legend([line for line in custom_lines],[label for label in mineralnames],
    ...
    ax1.text(0.985,0.945, letter , transform=ax1.transAxes, horizontalalignment = 'right',
             verticalalignment = 'baseline', fontweight = 'bold', fontsize = plot_parameters["size_fonts"],
