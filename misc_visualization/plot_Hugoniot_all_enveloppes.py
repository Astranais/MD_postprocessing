...
def main(argv):
    ...
    for opt,arg in options:
        if opt == '-h':
            ...
            print("plot_Hugoniot_all.py requires to be lauched from the folder containing also every Shock-state .txt file created by plot_Hugoniot_impedance-match.py")
            print('')
            sys.exit()
    ...
    for file in files:
        ...
        if gsT !='0':
            ax.plot(Hugoniot[xvariable],Hugoniot[yvariable], plot_params[mineral], markersize = 0, linewidth = plot_parameters["size_lines"], color =colors[gs])
        else:
    ...
    if yvariable == 'P' and xvariable == 'T':
        ...
        for gsT in Hugoniot_all:
            ax.fill_between(Xenveloppe[gsT],enveloppeUp[gsT],enveloppeDown[gsT], facecolor = colors_enveloppes[gsT], linewidth=plot_parameters['size_lines'], alpha = 0.1)
    else:
        #**** create the enveloppes
        for gsT in Xenveloppe:
            #complete the y data for each x (need to have the sale x axis)
        ...
        for gsT in Hugoniot_all:
            ...
            ax.fill_between(enveloppeDown[gsT]['x'],enveloppeUp[gsT]['y'],enveloppeDown[gsT]['y'], interpolate = True, facecolor = colors_enveloppes[gsT], linewidth=plot_parameters['size_lines'], alpha = 0.3)
    ...
    if (mineralname == 'all' or mineralname2 != '') and letter != '':
        ax.text(0.015,0.945, letter , transform=ax.transAxes, horizontalalignment = 'left', fontweight = 'bold', fontsize = plot_parameters["size_fonts"], bbox=dict(facecolor='none', edgecolor='k', pad=3.0))
    elif letter != '':
        ax.text(0.06,0.94, letter , transform=ax.transAxes, horizontalalignment = 'left', fontweight = 'bold', fontsize = plot_parameters["size_fonts"], bbox=dict(facecolor='none', edgecolor='k', pad=3.0))
    ...
    custom_markers = [Line2D([0], [0], marker = orderedmarkertype[ii], linestyle = '', markeredgecolor = 'k', markerfacecolor = 'k',  color = 'k', markersize = plot_parameters["size_markers"], markeredgewidth = 0.5, linewidth = plot_parameters["size_lines"]) for ii in range(len(orderedmarkertype))]
    legend3 = plt.legend([line for line in custom_markers],[str(label) + ' km/s' for label in natsort.natsorted(impactor_velocities)], title = 'Impactor velocity', bbox_to_anchor=(1.05, 0.31), loc=2, fontsize = plot_parameters["size_font_ticks"],  borderaxespad=0.)
