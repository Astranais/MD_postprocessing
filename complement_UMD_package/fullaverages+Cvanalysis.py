...
Langage: Python3
                ...
                ****     Compute the arithmetic average and stdev      ****
                ...
                + the relative temperature variance (check of the Cv values)
...
This code produces a thermo.txt file with:
    ...
    - 2 columns for the relative temperature variance (testCv) and statistical error'
"""
...
def selection_value(list_sigma, list_err):
    ...
    level_convergence = 6 #number of block transformation we want the sigma to be constant to say it converged
    stable = 0
    breakvalue = 0
    finalbreakvalue = 0
...
def figure_plot(file,list_sigma_rho, list_err_rho,list_sigma_P, list_err_P,list_sigma_T, list_err_T,list_sigma_E, list_err_E,final_sigma_rho,final_sigma_P,final_sigma_T,final_sigma_E):
    plt.close(1)
    ...
    ax1.set_ylabel('$\sigma_{rho}$ (g/cm3)', fontweight = 'bold', fontsize=12)
    ax2.set_ylabel('$\sigma_P$ (GPa)', fontweight = 'bold', fontsize=12)
    ax3.set_ylabel('$\sigma_T$ (K)', fontweight = 'bold', fontsize=12)
    ax4.set_ylabel('$\sigma_E$ (eV)', fontweight = 'bold', fontsize=12)
    # Fine-tune figure; make subplots close to each other and hide x ticks for all but bottom plot.
    fig.subplots_adjust(hspace=0, left=0.1, bottom=0.07, right=0.97, top=0.94)    
    #Plot data
    ax1.errorbar(np.arange(len(list_sigma_rho)),list_sigma_rho,yerr = list_err_rho, fmt='.')
    ax2.errorbar(np.arange(len(list_sigma_P)),list_sigma_P,yerr = list_err_P, fmt='.')
    ax3.errorbar(np.arange(len(list_sigma_T)),list_sigma_T,yerr = list_err_T, fmt='.')
    ax4.errorbar(np.arange(len(list_sigma_E)),list_sigma_E,yerr = list_err_E, fmt='.')
...
def main(argv):
    ...
    try:
        ...
        print('fullaverages+Cvanalysis.py -s <SkipSteps> -u <units version: 0 = minimal (default), 1 = full units>')
        sys.exit()
    for opt,arg in options:
        if opt == '-h':
            ...
            print('    - 2 columns for the relative temperature variance (testCv) and statistical error')
    ...
    for file in files:
        ...
        #**** Figure Plot
        #Creation of the figure, uncomment to save figures showing the convergence of the blocking method
        figure_plot(file,list_sigma_rho, list_err_rho,list_sigma_P, list_err_P,list_sigma_T, list_err_T,list_sigma_E, list_err_E,final_sigma_rho,final_sigma_P,final_sigma_T,final_sigma_E)
        ...
        # If we have E and KE, then we take both variances to compute the Cv
            ...
            for n in range(1000):
                #print('*****Bootstrap N°',n)
                Energy_rand = np.random.choice(Energy,nsteps_E,replace = True)
                Temperature_rand = np.random.choice(Temperature,nsteps_T,replace = True)
                variance_Erand = 0
                variance_Trand = 0
                variance_Erand = np.mean(Energy_rand**2) - np.mean(Energy_rand)**2
                variance_Trand = np.mean(Temperature_rand**2) - np.mean(Temperature_rand)**2
                Cv = variance_Erand  * (eVtoJ)**2 / (kb * Average_T**2)   #in J/unitcell/K
                testCv = variance_Trand / (Average_T**2)   #in J/unitcell/K
                Cv_bootstrap.append(Cv)
                testCv_bootstrap.append(testCv)
            Cv_bootstrap = np.asarray(Cv_bootstrap)    
            testCv_bootstrap = np.asarray(testCv_bootstrap)    
            Cv = np.mean(Cv_bootstrap) #an estimator of the mean of Cv, in J/unitcell/K
            testCv = np.mean(testCv_bootstrap) #an estimator of the mean of Cv, in J/unitcell/K
            Cv_stdev = np.sqrt(np.mean(Cv_bootstrap**2) - np.mean(Cv_bootstrap)**2 )  #the error on the previous mean
            testCv_stdev = np.sqrt(np.mean(testCv_bootstrap**2) - np.mean(testCv_bootstrap)**2 )  #the error on the previous mean
            
        #if we do not have the energy, then we do not compute the Cv
        else:
            print ('    WARNING !!! Cv is not computed')
            Cv, Cv_stdev = (0,0)
            testCv, testCv_stdev = (0,0)
