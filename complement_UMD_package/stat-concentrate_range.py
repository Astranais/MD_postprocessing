...
#written by Razvan
#modified by Anaïs to create files by taking into account a 'min-max' range of molecules length (min and max included)
...
def concatstatfile(FileName,population,iround,minlen,maxlen,lifetime):
    ...
    while True:
        ...
        if int(entry[3]) < maxlen and int(entry[3]) > minlen:         #only clusters smaller than maxlen and larger than minlen are considered, to be able to separate gas (i.e. small) from liquid (i.e. large polymers) species
            if float(entry[1]) > lifetime:            #only clusters with a total life time larger than lifetime are considered
...
def main(argv):
    ...
    try:
        ...
        print("stat-concentrate_range.py -f <output filename> -l <'min-max' range of molecules length> -t <min lifetime>  -r <polymerization_type>")
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
            print ("stat-concentrate_range.py program to concentrate all the stat files into one multi-column file")
            print ("stat-concentrate_range.py -f <output filename> -l <'min-max' range of molecules length (min and max included)> -t <min lifetime>  -r <polymerization_type>")
            sys.exit()
            ...
            minlen = int(arg.split('-')[0]) -1
            maxlen = int(arg.split('-')[1]) +1
    ...
    print('Minimum species size is ',minlen)
    print('Maximum species size is ',maxlen)
    if rings == 0:
    ...
    if len(list_of_files)>0:
        ...
        for ifile in range(len(list_of_files)):
            population = concatstatfile(list_of_files[ifile],population,ifile,minlen,maxlen,lifetime)
    else:
        print ('There are no *.stat.dat files to concatenate')
        sys.exit()
    minlen = minlen +1
    maxlen = maxlen -1
    outfile = logfile + 'abso-T' + str(lifetime) + '-L' + str(minlen)+'-'+str(maxlen) + '-R' + str(rings) + '.dat'
    percfile = logfile + 'perc-T' + str(lifetime) + '-L' + str(minlen)+'-'+str(maxlen) + '-R' + str(rings) + '.dat'
    ff = open(outfile,'w')
